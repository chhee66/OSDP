...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<a href="bbs4.jsp" class="btn btn-primary">목록</a>
			<%
				if (userID != null && userID.equals(bbs.getUserID())) { //작성자가 본인이라면 _19/5/25
			%>
			<a href="update4.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">수정</a>
			<!-- 매개변수로서 본인의 아이디를 가져감. _19/5/25-->
			<a onclick="return confirm('정말로 삭제하시겠습니까?')"href="deleteAction4.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
										                 <!-- -------------------------------------- 댓글이 보이는 부분 ----------------------------------------- -->
         ...
         <table class="table table-default">
            <%
                  ...
                  ArrayList<Comment4> list = commentDAO.getList(bbsID);
            ...
            <!-- --두번째 td-- -->
               <td>
                  ...
                  <form method="post" action="updateComment4.jsp?bbsID=<%=bbsID%>&modNum=<%=list.get(i).getCommentID()%>">
                  <div>
                     <!-- 내용부분 -->
               ...
               <td style="width: 10%">
                  ...
                  <div>
                     <a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteCommentAction4.jsp?commentID=<%=list.get(i).getCommentID() %>&bbsID=<%=bbsID %>" class="btn btn-default">삭제</a>
                  </div>

                  <div>
                     <a href="view4.jsp?bbsID=<%=bbsID %>&modNum=<%=list.get(i).getCommentID()%>#<%=list.get(i).getCommentID()%>" class="btn btn-default">수정</a>
                  </div>
               </td>
               <%
                     }
         ...
         <!-- -------------------------------------- 댓글을 쓰는부분 ----------------------------------------- -->
         <form method="post" action="writeCommentAction4.jsp?bbsID=<%=bbsID%>">
            <table class="table table-default">
               <tr>
