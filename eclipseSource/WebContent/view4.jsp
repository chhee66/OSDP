	...
	<div class="container">
		<div class="row">
			...
			<a onclick="return confirm('정말로 삭제하시겠습니까?')"
				href="deleteAction4.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
			<!-- -------------------------------------- 댓글이 보이는 부분 ----------------------------------------- -->
			...
			<table class="table table-default">
				...
				<tr>
					...
					<td>
						...
						<form method="post"
							action="updateComment4.jsp?bbsID=<%=bbsID%>&modNum=<%=list.get(i).getCommentID()%>">
							<div>
								<!-- 내용부분 -->
					...
					<td style="width: 10%">
						...
						<div>
							<a onclick="return confirm('정말로 삭제하시겠습니까?')"
								href="deleteCommentAction4.jsp?commentID=<%=list.get(i).getCommentID()%>&bbsID=<%=bbsID%>"
								class="btn btn-default">삭제</a>
						</div>

						<div>
							<a
								href="view4.jsp?bbsID=<%=bbsID%>&modNum=<%=list.get(i).getCommentID()%>#<%=list.get(i).getCommentID()%>"
