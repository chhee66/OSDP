		...
		<p>민원게시판</p>
			<table class="table table-striped"
				...
				<tbody>
					...
					<tr>
						<td><%=k-OrderNum%></td>
						<td><a href="view3.jsp?bbsID=<%=list.get(i).getBbsID()%>&modNum=<%=0%>#<%=0%>"><%=list.get(i).getBbsTitle().replaceAll(" ", "&nbsp").replaceAll("<", "&lt").replaceAll(">", "&gt").replaceAll("\n", "<br>")%></a></td>						<td><%=list.get(i).getUserID()%></td>
			...
			<%
				...
				if (bbs3DAO.nextPage(pageNumber+1)) { //다음 페이지가 존재한다면
			%>
